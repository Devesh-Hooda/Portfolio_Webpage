...
const LevelSystem: React.FC<LevelSystemProps> = ({ visitorName }) => {
  ...
  useEffect(() => {
    return scrollYProgress.on("change", (latest) => {
      ...
      else if (latest < 0.4) setStatus("ANALYZING HISTORY");
      else if (latest < 0.7) setStatus("PROCESSING RESOURCES");
      else if (latest < 0.9) setStatus("SYNCING ABILITIES");
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            ...
            <div className="h-1.5 w-full bg-slate-800 rounded-full overflow-hidden">
              <motion.div className="h-full bg-gradient-to-r from-purple-600 to-fuchsia-500" style={{ scaleX, transformOrigin: "left" }} />
            </div>
          </div>
